...
esp_err_t connect_handler(httpd_req_t* req) {
    ...
    if(err <= 0) {
        if (err == HTTPD_SOCK_ERR_TIMEOUT) {
            LOG_WARN("timeout on \"connect\" request, sending HTTP 408 (Request Timeout)");
...
httpd_uri_t uri_get_state_json = {
    .uri      = "/state",
    .method   = HTTP_GET,
