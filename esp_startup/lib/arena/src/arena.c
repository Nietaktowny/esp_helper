...
struct arena_obj {
    ...
    char* name;             // Arena name, used for debug purposes.
};
...
arena_t* arena_init(size_t size, size_t count, char* name)
{
    ...
    return arena_init_(size, count, buf, allocated, name);
}
...
arena_t* arena_init_(size_t size, size_t count, void* mem, size_t len, char* name)
{
    ...
    if(len < sizeof(arena_t) + size*count)
        return NULL;

    arena_t* a   = mem;
    struct node*  buf = (struct node*)((char*)mem + sizeof(arena_t));
...
static inline void* lazy_alloc(arena_t* a)
{
...
void arena_free(arena_t* a, void* p)
{
    ...
    n->next = a->free_list;
    a->free_list = n;
    LOG_DEBUG("free node %p in arena %s", p, a->name);
}
