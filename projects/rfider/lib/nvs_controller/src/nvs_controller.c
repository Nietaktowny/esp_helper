...
int nvs_c_init_partition(const char *partition_label) {
    ...
    if (err > NVS_PART_NAME_MAX_SIZE) {
        LOG_ERROR("nvs partition label exceeds maximum length, maximum is: %d, "
...
int nvs_c_open_from_partition(nvs_c_handle_t *handle, const char *partition_label, const char *namespace, nvs_c_open_mode_t mode) {
    ...
    Try {
        ...
        if (mode == NVS_C_READONLY) {
            LOG_VERBOSE("opened namespace \'%s\' in nvs partition '%s' for read only "
                        "operations",
                        namespace, partition_label);
        } else {
            LOG_VERBOSE("opened namespace \'%s\' in nvs partition '%s' for read and "
                        "write operations",
                        namespace, partition_label);
        }
    }
    Catch(err) {
        ...
        case ESP_ERR_NVS_NOT_FOUND:
            ...
            return NVS_C_ERR_NAMESPACE_NOT_FOUND;
        default:
            LOG_ERROR("error %d when trying to open namespace '%s' from "
                      "partition '%s': %s",
...
int nvs_c_write_string(nvs_c_handle_t handle, const char *const key, const char *buffer) {
    ...
    ERR_C_CHECK_ERROR(nvs_commit(handle->esp_nvs), LOG_ERROR("error %d when committing data with key '%s' into namespace "
                                                             "'%s': %s",
...
int nvs_c_read_string(nvs_c_handle_t handle, const char *key, char *buffer, size_t buflen) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        LOG_ERROR("buffer size (%zu bytes) is to small to store string data, "
                  "expected %zu bytes size.",
                  buflen, temp_buflen);
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        LOG_ERROR("error %d when writing data with key: \'%s\' into \'%s\' "
                  "namespace: %s",
...
int nvs_c_write_int8(nvs_c_handle_t handle, const char *const key, int8_t value) {
    ...
    ERR_C_CHECK_ERROR(nvs_set_i8(handle->esp_nvs, key, value), LOG_ERROR("error %d when writing data with key '%s' into namespace '%s': %s",
                                                                         err, key, handle->namespace, error_to_name(err)));
...
int nvs_c_write_uint8(nvs_c_handle_t handle, const char *const key, uint8_t value) {
    ...
    ERR_C_CHECK_ERROR(nvs_set_u8(handle->esp_nvs, key, value), LOG_ERROR("error %d when writing data with key '%s' into namespace '%s': %s",
                                                                         err, key, handle->namespace, error_to_name(err)));
...
int nvs_c_write_blob(nvs_c_handle_t handle, const char *const key, const void *data, size_t temp_buflen) {
    ...
    LOG_VERBOSE("writting blob data in namespace \'%s\' with key \'key\' and size: %zu", handle->namespace, key, temp_buflen);
    nvs_set_blob(handle->esp_nvs, key, data, temp_buflen);

    LOG_VERBOSE("written blob");
    ERR_C_CHECK_ERROR(nvs_commit(handle->esp_nvs), LOG_ERROR("error %d when committing data with key /'%s/' into namespace /'%s/': %s", err,
                                                             key, handle->namespace, error_to_name(err)));

    LOG_VERBOSE("written blob data bytes into namespace '%s'", handle->namespace);
    return err;
}
...
int nvs_c_read_blob(nvs_c_handle_t handle, const char *key, void *buffer, size_t buflen) {
    ...
    if (buflen == 0) {
        LOG_ERROR("buffer length cannot be '%zu', cannot read blob with key \'%s\'", buflen, key);
        return ERR_C_NO_MEMORY;
    }

    err = nvs_get_blob(handle->esp_nvs, key, NULL, &temp_buflen);

    LOG_VERBOSE("getting blob data length for key '%s' and size: %zu", key, buflen);
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        LOG_WARN("error %d, not found key \'%s\' within namespace \'%s\', error: %s", err, key, handle->namespace, error_to_name(err));
        return NVS_C_ERR_KEY_NOT_FOUND;
    } else if (temp_buflen != 0 && temp_buflen >= buflen) {
        LOG_ERROR("buffer size (%zu bytes) is to small to store blob data, expected "
                  "%lu bytes size.",
                  buflen, temp_buflen);
        return ERR_C_NO_MEMORY;
    }
    ...
    LOG_VERBOSE("read blob data bytes %zu from namespace: '%s'", temp_buflen, handle->namespace);
    return err;
}
...
int nvs_c_read_blob_length(nvs_c_handle_t handle, const char *key, size_t *out_datalen) {
    ...
    LOG_VERBOSE("getting blob data length for key \'%s\'...", key);
    ...
    LOG_VERBOSE("blob with key '%s' from namespace '%s' length is %zu", key, handle->namespace, *out_datalen);
    return err;
}
...
int nvs_c_read_string_length(nvs_c_handle_t handle, const char *key, size_t *out_datalen) {
    ...
    LOG_VERBOSE("string with key '%s' from namespace '%s' length is %zu", key, handle->namespace, *out_datalen);
    return err;
}
...
int nvs_c_erase_key(nvs_c_handle_t handle, const char *key) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        LOG_ERROR("error %d when erasing key '%s' from namespace '%s': %s", err, key, handle->namespace, error_to_name(err));
        return err;
    }

    ERR_C_CHECK_ERROR(nvs_commit(handle->esp_nvs),
                      LOG_ERROR("error %d when committing to namespace '%s': %s", err, handle->namespace, error_to_name(err)));
