 ...
 * @brief  Wifi manager server source file.
...
static esp_err_t ap_json_handler(httpd_req_t* req) {
	...
	ERR_C_CHECK_ERROR(wifi_manager_get_scanned_aps(buffer, sizeof(buffer)),
					  LOG_ERROR("cannot serve GET \"/scan\" HTTP request."));
...
static esp_err_t state_json_handler(httpd_req_t* req) {
	...
	ERR_C_CHECK_ERROR(wifi_c_get_status_as_json(buffer, sizeof(buffer)),
					  LOG_ERROR("cannot serve GET \"/state\" HTTP request."));
...
static esp_err_t clear_handler(httpd_req_t* req) {
	...
	if (err != ERR_C_OK) {
		LOG_ERROR("cannot erase stored AP in NVS, error %d : %s", err, error_to_name(err));
...
static httpd_uri_t uri_get_root = {.uri = "/", .method = HTTP_GET, .handler = root_handler, .user_ctx = NULL};

/* URI handler structure for GET /styles.css */
httpd_uri_t uri_get_styles = {
	.uri = "/styles.css", .method = HTTP_GET, .handler = stylesheet_handler, .user_ctx = NULL};

/* URI handler structure for GET /code.js */
httpd_uri_t uri_get_script = {.uri = "/code.js", .method = HTTP_GET, .handler = script_handler, .user_ctx = NULL};

/*URI handler structure for GET /scan */
httpd_uri_t uri_get_ap_json = {.uri = "/scan", .method = HTTP_GET, .handler = ap_json_handler, .user_ctx = NULL};

/*URI handler structure for GET /state */
httpd_uri_t uri_get_state_json = {.uri = "/state", .method = HTTP_GET, .handler = state_json_handler, .user_ctx = NULL};

/*URI handler structure for POST /connect */
httpd_uri_t uri_post_connect = {.uri = "/connect", .method = HTTP_POST, .handler = connect_handler, .user_ctx = NULL};

/*URI handler structure for GET /saved_ap */
httpd_uri_t uri_get_saved_ap_json = {
	.uri = "/saved_ap", .method = HTTP_GET, .handler = saved_ap_json_handler, .user_ctx = NULL};

/*URI handler structure for GET /restart */
httpd_uri_t uri_get_restart = {.uri = "/restart", .method = HTTP_GET, .handler = restart_handler, .user_ctx = NULL};

/*URI handler structure for GET /clear */
httpd_uri_t uri_get_clear = {.uri = "/clear", .method = HTTP_GET, .handler = clear_handler, .user_ctx = NULL};

/* -------------------------------*/
/**
 * @brief  Start wifi manager HTTP server.
 *
 * @param out_handle Wifi manager HTTP server handle.
 *
...
int wifi_manager_server_init(wifi_s_handle_t* out_handle) {
	...
	ERR_C_CHECK_NULL_PTR(out_handle, LOG_ERROR("address to store wifi manager http server handle cannot be NULL"));

	// alloc memory
	NEW((*out_handle), struct wifi_s_handle_obj);

	ERR_C_CHECK_ERROR(httpd_start(&temp_esp_handle, &default_config), LOG_ERROR("error %d during starting http server: %s", err, error_to_name(err)));
	...
	LOG_DEBUG("wifi manager http server started");
	return err;
}
