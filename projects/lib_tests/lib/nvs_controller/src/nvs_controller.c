...
int nvs_c_read_string(nvs_c_handle_t handle, const char *key, char *buffer, size_t buflen)
{
    ...
    {
        LOG_ERROR("buffer size (%llu bytes) is to small to store string data, expected %llu bytes size.", buflen, temp_buflen);
...
int nvs_c_read_blob(nvs_c_handle_t handle, const char *key, void *buffer, size_t buflen)
{
    ...
    {
        LOG_ERROR("buffer size (%llu bytes) is to small to store blob data, expected %llu bytes size.", buflen, temp_buflen);
        return ERR_C_NO_MEMORY;
    }
...
int nvs_c_erase_key(nvs_c_handle_t handle, const char *key)
{
    ...
    if (handle->mode == NVS_C_READONLY)
    {
        LOG_WARN("cannot erase key, namespace '%s' was opened in read only mode");
        return NVS_C_ERR_WRONG_MODE;
    }
...
int nvs_c_erase_partition(const char *partition_label)
{
    ...
    {
        LOG_ERROR("error %d when trying to erase '%s' nvs partition", partition_label);
        return err;
    }
