...
int adc_c_init_oneshot(adc_c_oneshot_handle_t *handle, adc_c_gpio_t gpio) {
    ...
    ERR_C_CHECK_NULL_PTR(handle, LOG_ERROR("location to store handle cannot be NULL"));

    NEW(*handle, struct adc_c_oneshot_handle_obj);

    /*
     * if number of ADC gpio is bigger than number of all GPIOS in chip
     * then adc_oneshot_io_to_channel return ESP_ERR_INVALID_ARG instead of ESP_ERR_NOT_FOUND
     * manualy check it and return correct error
     */
    // translate gpio to channel
    err = adc_oneshot_io_to_channel(gpio, &((*handle)->esp_config.unit_id), &channel);
    if ((err != ERR_C_OK && err == ESP_ERR_NOT_FOUND) || gpio >= SOC_GPIO_PIN_COUNT) {
        LOG_ERROR("error %d, passed GPIO %d is not a valid ADC pad.", err, gpio);
        return ADC_C_ERR_WRONG_GPIO;
    } else if (err != ERR_C_OK) {
        LOG_ERROR("error %d when translating GPIO %d to channel: %s", err, gpio, error_to_name(err));
    ...
    err = adc_oneshot_new_unit(&((*handle)->esp_config), &((*handle)->esp_handle));
    if (err != ERR_C_OK && err == ESP_ERR_NOT_FOUND) {
        err = ADC_C_ERR_ADC_ALREADY_INIT;
        LOG_WARN("error %d, the ADC unit %d is already in use.", err, (*handle)->esp_config.unit_id);
    } else if (err != ERR_C_OK) {
        LOG_ERROR("error %d when trying to init oneshot adc: %s", err, error_to_name(err));
    ...
    ERR_C_CHECK_ERROR(adc_oneshot_config_channel((*handle)->esp_handle, channel, &channel_config),
                      LOG_ERROR("error %d when trying trying to config adc channel: %s", err, error_to_name(err)));

    ERR_C_CHECK_ERROR(adc_c_oneshot_calibrate(*handle),
                      LOG_ERROR("error %d when calibrating ADC channel %s: %s", err, adc_c_translate_channel(channel), error_to_name(err)));
...
int adc_c_deinit_oneshot(adc_c_oneshot_handle_t *handle) {
    ...
    ERR_C_CHECK_NULL_PTR(*handle, LOG_ERROR("handle was not initalized, cannot be NULL"));
    err = adc_oneshot_del_unit((*handle)->esp_handle);
    if (err != ERR_C_OK) {
        LOG_ERROR("error %d during oneshot adc handle deinit: %s", err, error_to_name(err));
    }
    ...
    if (err != ERR_C_OK && err == ESP_ERR_NOT_FOUND) {
        ...
        LOG_ERROR("error %d, the ADC peripheral to be disclaimed isn't in use.", err);
    } else if (err != ERR_C_OK) {
        LOG_ERROR("error %d during oneshot calibration handle deinit: %s", err, error_to_name(err));
    }
...
int adc_c_oneshot_calibrate(adc_c_oneshot_handle_t handle) {
    ...
    ERR_C_CHECK_ERROR(adc_cali_create_scheme_curve_fitting(&cali_config, &((*handle).esp_cali)),
                      LOG_ERROR("error %d when creating curve fitting calibration scheme: %s", err, error_to_name(err)));
    ...
    ERR_C_CHECK_ERROR(adc_cali_create_scheme_line_fitting(&cali_config, &((*handle).esp_cali)),
                      LOG_ERROR("error %d when creating line fitting calibration scheme: %s", err, error_to_name(err)));
...
int adc_c_oneshot_read_channel(adc_c_oneshot_handle_t handle, adc_c_channel_t channel, int *out) {
    err_c_t err = ERR_C_OK;
    int raw = -1;

    ERR_C_CHECK_NULL_PTR(handle, LOG_ERROR("ADC handle cannot be NULL"));
    ERR_C_CHECK_NULL_PTR(out, LOG_ERROR("location to store ADC result cannot be NULL"));

    err = adc_oneshot_read(handle->esp_handle, channel, &raw);
    if (err != ERR_C_OK && err == ESP_ERR_TIMEOUT) {
        LOG_WARN("error %d, cannot obtain result, timeout occured: %s", err, error_to_name(err));
        return err;
    } else if (err != ERR_C_OK && err != ESP_ERR_TIMEOUT) {
        LOG_ERROR("error %d during reading oneshot raw ADC result: %s", err, error_to_name(err));
        return err;
    }

    LOG_VERBOSE("read ADC raw value: %d from channel: %s", raw, adc_c_translate_channel(channel));

    // calculate voltage according to docs
    ERR_C_CHECK_ERROR(adc_cali_raw_to_voltage(handle->esp_cali, raw, out),
                      LOG_ERROR("error %d when translating raw ADC values to voltage: %s", err, error_to_name(err)));
...
int adc_c_oneshot_read_gpio(adc_c_oneshot_handle_t handle, adc_c_gpio_t gpio, int *out) {
    ...
    Try {
        ...
        // translate gpio to channel
        ERR_C_CHECK_AND_THROW_ERR(adc_oneshot_io_to_channel(gpio, &(handle->esp_config.unit_id), &channel));

        ERR_C_CHECK_AND_THROW_ERR(adc_oneshot_get_calibrated_result(handle->esp_handle, handle->esp_cali, channel, out));
