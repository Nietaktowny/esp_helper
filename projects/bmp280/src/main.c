...
#define SERVER_READINGS_INPUT_ACTION "modules/setters/insert_data.php"
...
void read_temperature_task(void *args) {
  ...
  LOG_INFO("Identified BMP280 device:\n address: %x\n chip id: %x\n mode: %x",
           bmp_get_i2c_addr(bmp), bmp_get_chip_id(bmp), bmp_get_mode(bmp));

  err = http_client_init_reuse(&client, SERVER_ADDRESS,
                               SERVER_READINGS_INPUT_ACTION);
  if (err != ERR_C_OK) {
    LOG_ERROR(
        "error %d when trying to prepare http_client handle for reuse: %s", err,
        error_to_name(err));
    //    vTaskDelete(NULL);
  }
  ...
  while (1) {
    ...
    altitude[iteration] = bmp_i2c_calculate_altitude(
        pressure[iteration] * 0.01, BMP_STANDARD_SEA_LEVEL_PRESSURE);
    ...
    if (iteration < BMP_NUMBER_OF_VALUES_TO_READ) {
      LOG_VERBOSE("number of already collected BMP280 values: %d / %d",
                  iteration, BMP_NUMBER_OF_VALUES_TO_READ);
      iteration++;
    ...
    for (uint8_t i = 0; i < BMP_NUMBER_OF_VALUES_TO_READ; i++) {
      ...
      LOG_VERBOSE("read temperature value number %d is: %.2f C", i,
                  temperature[i]);
      temperature_avg += temperature[i];
      LOG_VERBOSE("read altitude value number %d is: %.2f m. n. p. m.", i,
                  altitude[i]);
    ...
    pressure_avg = pressure_avg /
                   BMP_NUMBER_OF_VALUES_TO_READ; // we start from 0, so add 1 to
                                                 // make it the same as
                                                 // BMP_NUMBER_OF_VALUES_TO_READ
    ...
    sprintf(post_data,
            "device_id=%s&temperature=%.2f&pressure=%.2f&altitude=%.2f",
            device_id, temperature_avg, pressure_avg * 0.01, altitude_avg);
    LOG_VERBOSE("data to send to database: %s", post_data);
    err =
        http_client_post_reuse(client, post_data, HTTP_CLIENT_POST_USE_STRLEN);
    if (err != ERR_C_OK) {
      ...
      err = http_client_init_reuse(&client, SERVER_ADDRESS,
                                   SERVER_READINGS_INPUT_ACTION);
      if (err != ERR_C_OK) {
        LOG_ERROR("error %d when trying to reestablish connection: %s", err,
                  error_to_name(err));
        vTaskDelay(pdMS_TO_TICKS(BMP_DELAY_BETWEEN_READ));
        continue;
      }
      err = http_client_post_reuse(client, post_data,
...
void on_connect_handler(void) {
  ...
  xTaskCreate(read_temperature_task, "log_task", 8096, (void *)i2c_bus, 1,
              NULL);
